...
$mpdf->SetFooter(tgl(date('Y - m - d')).'||Halaman {PAGENO} dari {nb}');
$html = '
<html>
    <head>
    ...
    <h4 align="center">Metode <i>Simple Additive Weighting</i> (SAW)</h4>';
    $n = mysql_fetch_array(mysql_query("SELECT * FROM t_karyawan WHERE nik='".$id."' AND status='Y'"));
    $html.='<h4 align="center">Karyawan : '.$n['nama'].'</h4><br/><br/> 
    <h4>1. Penilaian</h4> 
    <table width="50%" border="1" cellpadding="10" cellspacing="10" style="border-collapse:collapse" bordercolor="#CCCCCC">
        ...
        $q="SELECT n.*,k.nama,s.nilai FROM t_kriteria k, t_nilai n, t_standar s "
                . "WHERE k.id_kriteria = n.id_kriteria and n.id_standar=s.id_standar "
                . "and n.nik='".$id."' and k.status='Y' and n.status='Y' order by k.nama";
        $sql = mysql_query($q);
        $no=1;
...
$html.='
        ...
        <table width="50%" border="1" cellpadding="10" cellspacing="10" style="border-collapse:collapse;" bordercolor="#CCCCCC">
            ...
            $no = 1;
            $sql = mysql_query("SELECT k.nama,k.id_kriteria,k.atribut FROM t_kriteria k left join t_bobot t "
                    . "on k.id_kriteria=t.id_kriteria where k.status='Y' ORDER BY k.nama");
            while ($row = mysql_fetch_array($sql)) {
                $html.='<tr>
                            <td>'.$no++.'</td>
                            <td>'.$row['nama'].'</td>
                ...
                if ($row['atribut'] == 'C') {
                    $m = mysql_query("select min(s.nilai) as nilai from t_nilai n, t_standar s, t_kriteria k "
                            . "where n.id_standar=s.id_standar and n.id_kriteria=k.id_kriteria and n.status='Y' "
                            . "and s.status='Y' and k.status='Y' and n.id_kriteria='".$row['id_kriteria']."'");
...
$html.='</table>
    ...
    $no = 1;
    $sqls = mysql_query("SELECT n.*,k.nama,s.nilai FROM t_kriteria k, t_nilai n, t_standar s "
            . "WHERE k.id_kriteria = n.id_kriteria and n.id_standar=s.id_standar and "
            . "n.nik='".$id."' and k.status='Y' and n.status='Y' order by k.nama");
    while ($rows = mysql_fetch_array($sqls)) {
           $html.='<tr>
                ...
                $ys = mysql_query("select n.*,s.*,k.* from t_nilai n, t_standar s, t_kriteria k "
                        ...
                        . "and n.nik='" . $rows['nik'] . "' and n.id_kriteria='". $rows['id_kriteria']."' "
                        . "and n.status='Y' and s.status='Y' and k.status='Y' order by k.nama");
                $yy = mysql_fetch_array($ys); 
                if ($yy['atribut'] == 'C'){
                    $m = mysql_query("select min(s.nilai) as nilai from t_nilai n, t_standar s, t_kriteria k "
                            . "where n.id_standar=s.id_standar and n.id_kriteria=k.id_kriteria and n.status='Y' "
                            . "and s.status='Y' and k.status='Y' and n.id_kriteria='".$yy['id_kriteria']."'");
                    $min = mysql_fetch_array($m);
...
$html.='</table>
        ...
        $sqlr = mysql_query("SELECT k.* FROM t_karyawan k, t_nilai n "
                . "WHERE k.nik = n.nik and k.status='Y' and n.status='Y' and k.nik='".$id."'"
        ...
        while ($rowr = mysql_fetch_array($sqlr)) {
            ...
            while ($yyr = mysql_fetch_array($yr)) {
                if ($yyr['atribut'] == 'C') {
                    ...
                    $mr = mysql_query("select max(s.nilai) as nilai "
                            . "from t_nilai n, t_standar s, t_kriteria k "
                            . "where n.id_standar=s.id_standar and n.id_kriteria=k.id_kriteria "
                            . "and n.status='Y' and s.status='Y' and k.status='Y' "
                            . "and n.id_kriteria='" . $yyr['id_kriteria'] . "'");
...
$mpdf->Output("laporan-penilaian-".$id.".pdf",'I');
exit;
?>